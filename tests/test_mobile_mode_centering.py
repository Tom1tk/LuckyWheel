...
def server_url():
    """Spawn the Flask server on a free port. Module-scoped so the
    single test shares one process (server boot is ~2s)."""
    port = _free_port()
    env = os.environ.copy()
    env['PORT'] = str(port)
    env.setdefault('WHEEL_SECRET_KEY', 't206-test-secret-key-for-playwright-only')
...
# test_mobile_e2e.py + test_mobile_drawer_style.py). The single test
# gets a fresh chromium browser instance (function-scoped) so context
# and page don't leak.
