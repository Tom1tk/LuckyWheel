...
_wheel_modes = None
_resolve_spin = None
_current_wheel_probabilities = None
...
def setup_module(module):
    ...
    test in this module."""
    global _game, _wheel_modes, _resolve_spin, _current_wheel_probabilities
    for name, factory in _stub_specs():
        _STUB_PREV[name] = sys.modules.get(name, _SENTINEL)
        sys.modules[name] = factory()
    # The `db` stub needs the locally-defined _fake_db_connection.
    ...
    # wheel_modes constants consumed by the test bodies
    global GRAVITY_DRIFT_STEP, GRAVITY_DRIFT_MAX, GRAVITY_DRIFT_MIN
...
def teardown_module(module):
    ...
    sys.modules.pop('game', None)
    sys.modules.pop('wheel_modes', None)
    _game = _wheel_modes = _resolve_spin = _current_wheel_probabilities = None
...
GRAVITY_DRIFT_STEP = None  # set in setup_module
GRAVITY_DRIFT_MAX  = None
GRAVITY_DRIFT_MIN  = None
