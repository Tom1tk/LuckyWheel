...
from community_goals import (
    get_active_goal, increment_goal, check_goal_completion,
)
...
def lure_level(owned) -> int:
    ...
    """
    for lvl, item in [(5, 'lure_5'), (4, 'lure_4'), (3, 'lure_3'),
                      (2, 'lure_2'), (1, 'lure_1')]:
        if item in owned:
            return lvl
    return 0
...
def autofisher_level(owned) -> int:
    ...
    """
    for lvl, item in [(4, 'autofisher_4'), (3, 'autofisher_3'),
                      (2, 'autofisher_2'), (1, 'autofisher_1')]:
        if item in owned:
            return lvl
    return 0
...
def cast_line(cur, user_id: int, now_utc: dt.datetime,
              *, rand=None) -> dict | tuple[int, dict]:
...
def reel_line(cur, conn, user_id: int, now_utc: dt.datetime,
              *, rand=None) -> dict | tuple[int, dict]:
    ...
    species_id     = roll_fish(auto_mode=False, master_lure=(lure_lvl >= 5),
    ...
    new_ewma       = precise_pct if old_ewma is None else (
        _EWMA_ALPHA * precise_pct + (1 - _EWMA_ALPHA) * old_ewma
    )
    ...
    if precise_pct < 12.0:
        ...
        if new_suspicious % 10 == 0:
            log.warning('SUSPICIOUS_REEL user_id=%s pct=%.1f ewma=%.1f '
                        'catch_count=%d suspicious=%d',
                        user_id, precise_pct, new_ewma, new_catch_count,
                        new_suspicious)
    ...
    if catch_of_day_bonus:
        ...
        cur.execute(
            '''UPDATE game_state
               ...
               WHERE user_id = %s''',
            (new_fish_clicks, new_lucky_next, caught_species, new_best,
             new_suspicious, new_catch_count, new_ewma, user_id),
        )
...
def auto_fish_tick(cur, conn, user_id: int, now_utc: dt.datetime,
                   *, rand=None) -> dict | tuple[int, dict]:
...
def set_auto_fish_enabled(cur, conn, user_id: int, requested: bool,
...
def _post_catch_bookkeeping(conn, user_id: int, now_utc: dt.datetime,
                            first_catch: bool) -> None:
    ...
    season_num = (season_info.get('season_number', 8)
                  if season_info else 8)
